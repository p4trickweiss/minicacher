...
describe("Eventual Consistency", () => {
  test("should eventually replicate data to all nodes", async () => {
    await waitForAllNodesHealthy();

    const key = uniqueKey("eventual");
    const value = "eventually-consistent";

    // Write to leader
    const leader = await findLeader();
    await setKey(leader!.url, key, value);

    // Poll until all nodes have the data (max 5 seconds)
    const maxAttempts = 10;
    let consistent = false;

    for (let i = 0; i < maxAttempts; i++) {
      await sleep(500);
      consistent = await verifyKeyOnAllNodes(key, value);
      if (consistent) {
        console.log(`Achieved consistency after ${(i + 1) * 500}ms`);
        break;
      }
    }
  ...
  test("should measure replication latency", async () => {
    await waitForAllNodesHealthy();

    const key = uniqueKey("latency");
    const value = "measure-replication";

    const leader = await findLeader();

    // Write and measure time until all nodes have it
    const writeStart = Date.now();
    await setKey(leader!.url, key, value);

    // Poll until replicated
    while (Date.now() - writeStart < 5000) {
      const allMatch = await verifyKeyOnAllNodes(key, value);
      if (allMatch) {
        const latency = Date.now() - writeStart;
        console.log(`Replication latency: ${latency}ms`);
        expect(latency).toBeLessThan(3000); // Should replicate within 3 seconds
        return;
      }
      await sleep(100);
    }

    throw new Error("Replication did not complete within 5 seconds");
  });
});
